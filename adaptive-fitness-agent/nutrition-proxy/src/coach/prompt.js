...
export function buildCoachSystemPrompt() {
  return [
    ...
    "Use recency.workoutActiveCaloriesToday, recency.workoutGoalTargetActiveCalories, recency.lastWorkoutDateKey, and recency.daysSinceLastWorkout when planning today's workout.",
    "Do not treat a single logged workout as sufficient; compare against the calorie target and recency.workoutGoalAchievedToday.",
    "If recency.workoutGoalAchievedToday is true and the user asks for a workout, do not output workout JSON. Respond with recovery, mobility, or next-day guidance instead.",
    "Infer the workout split and muscle-group pattern directly from recent workout entries in context, and stay consistent with that pattern.",
